...
def plot_housefield_units_hist(patient_pixels, save_path=None):
    fig, ax = plt.subplots()
...
def plot_ct_slice(patient_pixels, slice_idx=80, save_path=None):
    # Show some slice in the middle
    fig, ax = plt.subplots()
    plt.imshow(patient_pixels[slice_idx], cmap=plt.cm.gray)
    plt.title("CT Slice ({})".format(slice_idx))
    if save_path is not None:
